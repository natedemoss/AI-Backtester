...
class Backtester:
    ...
    def run(self) -> Dict:
        ...
        for index, row in self.data.iterrows():
            ...
            if row['signal'] == 1 and not position_open:  # Buy signal
                current_position = self._execute_buy(index, row['Close'])
                position_open = True
            elif row['signal'] == -1 and position_open:  # Sell signal
    ...
    def _execute_buy(self, date, price: float) -> Dict:
        ...
        print(f"BUY: {date.date()} at ${price:.2f} (Cost: ${cost:.2f})")
        return position
    ...
    def _execute_sell(self, date, price: float, position: Dict):
        ...
        print(f"SELL: {date.date()} at ${price:.2f} (Profit: ${profit:.2f})")
